...
// ANTI-ERROR VERCEL: Hindari penggunaan Regex yang rumit
...
// INJEKSI SCRIPT SHEETJS (ANTI VERCEL BUILD ERROR)
...
const modalOverlay = "fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md transition-opacity";
const modalContent = "bg-slate-900 border border-slate-700 shadow-[0_0_50px_rgba(0,0,0,0.5)] rounded-[2.5rem] w-full max-w-lg relative z-10 p-8 flex flex-col max-h-[90vh] overflow-y-auto scrollbar-hide transform transition-transform duration-300 scale-100";

// ==========================================
// 3. KOMPONEN MODULAR INTERNAL (ANTI ERROR)
// ==========================================

// --- MODUL MANAJEMEN DATA (EXCEL & RESET) ---
  ...
  return (
    <div className="space-y-6">
      <div className={`${glassCard} p-8 flex flex-col lg:flex-row justify-between items-center gap-6 overflow-hidden relative`}>
        ...
        <div className="flex bg-slate-800/80 p-1.5 rounded-2xl relative z-10 border border-slate-700 overflow-x-auto">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-center mb-8 gap-4">
          ...
          {isKorkab && (
            <div className="flex gap-3 w-full lg:w-auto">
              <label className="cursor-pointer bg-slate-800 hover:bg-slate-700 border border-slate-600 text-cyan-400 px-5 py-3 rounded-xl font-bold flex items-center gap-2 transition-all"><UploadCloud className="w-5 h-5" /> Import Excel<input type="file" accept=".csv, .xlsx, .xls" className="hidden" onChange={handleFileUpload} disabled={loading}/></label>
              <button onClick={handleAddColumn} className="bg-slate-800 hover:bg-slate-700 border border-slate-600 text-blue-400 px-5 py-3 rounded-xl font-bold flex items-center gap-2 transition-all cursor-pointer"><Columns className="w-5 h-5" /> Kolom</button>
              <button onClick={() => { setCurrentRecordId(null); setFormData({}); setModalMode('add'); setShowModal(true); }} className="bg-cyan-500 hover:bg-cyan-400 text-slate-900 px-5 py-3 rounded-xl font-bold flex items-center gap-2 shadow-[0_0_15px_rgba(34,211,238,0.4)] transition-all cursor-pointer"><Plus className="w-5 h-5" /> Baris</button>
            </div>
          )}
        </div>
        ...
        <div className={modalOverlay} onClick={() => setShowModal(false)}>
          <div className={modalContent} onClick={e => e.stopPropagation()}>
            ...
            <form id="crud-form" onSubmit={handleSaveData} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {(headers.length > 0 ? headers : ['nama', 'nik', 'status']).map((key) => (<div key={key}><label className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1 mb-1 block">{key.replace(/_/g, ' ')}</label><input type="text" value={formData[key] || ''} onChange={(e) => setFormData({ ...formData, [key]: e.target.value })} className={glassInput} /></div>))}
            </form>
...
export default function App() {
  ...
  const handleMenuClick = (mId, subs) => { if(subs?.length) setExpandedMenu(expandedMenu===mId ? '' : mId); else {setActiveTab(mId); setActiveSubTab(''); setIsSidebarOpen(false); setSelectedKPM(null);} };
  ...
  const dbAdd = async (coll, data) => { setIsSaving(true); try { if(db) await set(push(ref(db, getBasePath(coll))), data); showToast("Data Tersimpan ke Cloud!"); } catch(e){} finally { setIsSaving(false); setModals({sdm:false, tugas:false, vote:false, pengaduan:false, catatan:false, app:false, potensial:false, graduasi:false, agenda:false}); } };
  const dbDelete = async (coll, id) => { try { if(db) await remove(ref(db, `${getBasePath(coll)}/${id}`)); showToast("Data Dihapus Permanen!"); } catch(e){} };

  // --- MENU CONFIGURATIONS MENGGUNAKAN REFERENSI KOMPONEN ---
  const menuConfig = [
  ...
  if (isLoggedIn === 'false' || !isLoggedIn) return (
    <div className="min-h-screen bg-slate-950 flex justify-center items-center relative overflow-hidden font-sans">
      ...
      <div className={`${glassCard} p-10 lg:p-14 w-full max-w-md relative z-10 transition-transform duration-500 transform scale-100 border border-slate-700/50 shadow-[0_0_50px_rgba(0,0,0,0.5)]`}>
        ...
        {loginError && (<div className="bg-rose-500/10 text-rose-400 p-4 rounded-2xl text-xs font-bold mb-6 flex items-center border border-rose-500/20"><AlertCircle className="w-5 h-5 mr-3 shrink-0" /> {String(loginError)}</div>)}
    ...
    <div className="flex h-screen bg-slate-950 text-slate-200 overflow-hidden font-sans relative">
      ...
      <aside className={`fixed inset-y-4 left-4 z-50 w-[280px] bg-slate-900/60 backdrop-blur-3xl border border-slate-700/50 shadow-2xl rounded-[2.5rem] transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-[120%]'} lg:relative lg:translate-x-0 transition-all duration-500 flex flex-col overflow-hidden`}>
        ...
        <nav className="flex-1 overflow-y-auto px-4 py-6 space-y-2 scrollbar-hide">
          {menuConfig.map(item => {
             ...
             const hasSub = item.subMenu?.length > 0; const isExpanded = expandedMenu === item.id; const isActive = activeTab === item.id && !hasSub; const isParentActive = activeTab === item.id || (hasSub && item.subMenu.some(s => s.id === activeSubTab && activeTab === item.id));
      ...
      <div className="flex-1 flex flex-col min-w-0 relative z-10">
        <header className="h-24 flex items-center justify-between px-6 lg:px-10">
          <div className="flex items-center"><button onClick={() => setIsSidebarOpen(true)} className="lg:hidden mr-4 p-3 bg-slate-800 border border-slate-700 text-slate-300 rounded-xl cursor-pointer"><Menu className="w-5 h-5" /></button><h1 className="font-black text-2xl capitalize tracking-tight text-white hidden sm:flex items-center gap-3">{activeTab.replace('_', ' ')} {activeSubTab && <><ChevronRight className="w-5 h-5 text-cyan-500 opacity-50"/> <span className="text-cyan-400">{activeSubTab.replace('_', ' ')}</span></>}</h1></div>
        ...
        <main className="flex-1 overflow-y-auto p-4 lg:p-8 scrollbar-hide pb-24">
          ...
          {activeTab === 'dashboard' && (
            <div className="space-y-6 transition-all transform translate-y-0 opacity-100 duration-700 max-w-7xl mx-auto">
              <div className={`${glassCard} p-8 lg:p-12 overflow-hidden relative border-cyan-500/20`}>
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                  <div>
                    ...
                    <p className="text-slate-400 mt-4 text-lg font-medium max-w-md">Sistem manajemen cerdas untuk produktivitas maksimal pendamping PKH.</p>
                  </div>
          ...
          {/* --- MANAJEMEN DATA (MASTER EXCEL) --- */}
          {activeTab === 'manajemen_data' && isKorkab && <ModulManajemenData db={db} isKorkab={isKorkab} showToast={showToast} />}

          {/* --- KPM --- */}
          {activeTab === 'kpm' && (
             <div className="space-y-6 max-w-7xl mx-auto transition-all transform translate-y-0 opacity-100 duration-500">
          ...
          {!['dashboard', 'kpm', 'agenda', 'manajemen_data'].includes(activeTab) && (
            <div className={`${glassCard} p-12 text-center flex flex-col items-center justify-center min-h-[60vh] transition-all transform translate-y-0 opacity-100 duration-500 max-w-4xl mx-auto mt-10 border-t-4 border-t-blue-500/50`}>
              ...
              <h2 className="text-4xl font-black text-white mb-4 tracking-tight">Modul <span className="text-cyan-400">{activeTab.replace('_', ' ').toUpperCase()}</span></h2>
              <p className="text-slate-400 font-medium max-w-lg mx-auto text-lg leading-relaxed">
              ...
              <button onClick={() => goToMenu('dashboard')} className="mt-10 px-10 py-5 bg-slate-800 border border-slate-600 text-white font-black rounded-2xl hover:bg-slate-700 hover:border-slate-500 transition-all cursor-pointer shadow-lg hover:shadow-xl uppercase tracking-widest text-sm">Kembali ke Dashboard</button>
            </div>
          )}
        </main>
      </div>
      ...
      {toastMessage && (
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 bg-slate-900/90 backdrop-blur-2xl text-white px-8 py-5 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] border border-slate-700 flex items-center gap-4 font-black z-[9999] transition-all duration-500 transform translate-y-0 opacity-100 min-w-[300px] justify-center">
